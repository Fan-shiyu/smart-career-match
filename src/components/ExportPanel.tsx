...
export function ExportPanel({ jobs }: ExportPanelProps) {
  ...
  const handleExport = () => {
    ...
    switch (preset) {
      case "quick":
        headers = ["Title", "Company", "City", "Salary", "Match Score", "Visa", "Commute (min)", "Apply URL"];
        getData = (j) => [
        ...
        break;
      case "detailed":
        headers = ["Title", "Company", "City", "Country", "Mode", "Type", "Salary", "Skills", "Tools", "Match", "Visa", "Commute", "Source", "Posted", "Apply URL"];
        getData = (j) => [
          j.job_title, j.company_name, j.city || "", j.country, j.work_mode || "", j.employment_type || "",
          j.salary_min && j.salary_max ? `${j.salary_min}-${j.salary_max}` : "",
          j.hard_skills.join("; "), j.software_tools.join("; "),
          String(j.match_score_overall), j.visa_likelihood || "",
          j.commute_time_min != null ? String(j.commute_time_min) : "",
          j.source, j.date_posted, j.apply_url
        ];
        break;
      case "skill-gap":
        headers = ["Title", "Company", "Hard Skills", "Matched", "Missing", "Match Score", "Skills Score", "Tools Score"];
        getData = (j) => [
          j.job_title, j.company_name, j.hard_skills.join("; "),
          j.matched_skills.join("; "), j.missing_skills.join("; "),
          String(j.match_score_overall),
          String(j.match_score_breakdown.hard_skills),
          String(j.match_score_breakdown.tools)
        ];
        break;
      default:
