...
// Category colors for Excel headers (ARGB)
...
export function ExportPanel({ jobs }: ExportPanelProps) {
  ...
  const handleExport = async (format: "csv" | "xlsx" = "csv") => {
    ...
    if (format === "xlsx") {
      await exportXlsx(columns as typeof ALL_COLUMNS, jobs);
    } else {
      exportCsv(columns as typeof ALL_COLUMNS, jobs);
    }

    await trackEvent("export");
  };

  const exportCsv = (columns: typeof ALL_COLUMNS, data: Job[]) => {
    ...
    downloadBlob(blob, `job-intelligence-${preset}-${dateStr()}.csv`);
  };

  const exportXlsx = async (columns: typeof ALL_COLUMNS, data: Job[]) => {
    ...
    // Build category groups for merged header row
    ...
    columns.forEach((col, idx) => {
      if (col.category !== currentCat) {
        if (currentCat) {
          categoryGroups.push({ category: currentCat, startCol, endCol: idx });
        }
    ...
    if (currentCat) {
      categoryGroups.push({ category: currentCat, startCol, endCol: columns.length });
    }
    ...
    // Row 2: Column names with category-tinted background
    columns.forEach((col, idx) => {
    ...
    // Data rows
    data.forEach((job) => {
      const row = columns.map((c) => c.getValue(job));
      ws.addRow(row);
    });
    ...
    columns.forEach((_, idx) => {
      ...
      data.forEach((job) => {
        const val = columns[idx].getValue(job);
    ...
    if (limits.plan === "free") {
      const watermarkRow = ws.addRow(columns.map((_, i) => i === 0 ? "Generated by Job Intelligence — Upgrade for full export" : ""));
      watermarkRow.font = { italic: true, color: { argb: "FF999999" } };
    }
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => handleExport("xlsx")} className="h-8 text-xs gap-1.5">
...
function lightenArgb(argb: string): string {
  // Make ARGB color lighter by blending with white
